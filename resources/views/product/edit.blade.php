<x-meta>
    ...
    <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <input type="text" name="quantity" id="visitors"
            class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-2.5 py-2 shadow-xs placeholder:text-body"
        ...
        <div class="flex items-center mb-4">
            <input id="default-radio-1" {{ old('category', $product->category) === 'male' ? 'checked' : '' }}
                type="radio" value="male" name="category"
        ...
        <div class="flex items-center">
            <input id="default-radio-2" {{ old('category', $product->category) === 'female' ? 'checked' : '' }}
                type="radio" value="female" name="category"
                class="w-4 h-4 text-neutral-primary border-default-medium bg-neutral-secondary-medium rounded-full checked:border-brand focus:ring-2 focus:outline-none focus:ring-brand-subtle border border-default appearance-none">
